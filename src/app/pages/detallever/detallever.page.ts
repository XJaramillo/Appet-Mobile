...
export class DetalleverPage implements OnInit {
  ...
  registrarComentario() {
    ...
    alert("Ahora puedes calificar al usuario, los comentarios serán verificados por administradores de APPET");
      ...
      this.auth.isAuth().subscribe(user => {
        ...
        if (this.c.length == 0) {
          if (this.articulo.userId == this.user) {
            alert("Servicio de su propiedad no puede contactarse a usted mismo");
            this.regresar();
          } else {
